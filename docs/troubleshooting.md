...
| Migration waits for a lock | Check `list-locks` and active migration jobs. Force-release only after confirming the owner is gone. A timeout does not release another process's lock. |
